...
test_that("Netherlands returns correct HIV population size", {
  ...
  expect_equal(round(colSums(r$data[,,2,],,2), 6), round(colSums(c$data[,,2,],,2), 6))
  expect_equal(round(colSums(c$data[,,2,],,2), 6), hivpop_mod)
  expect_equal(round(colSums(r$data[,,2,],,2), 6), hivpop_mod)
})

test_that("hivpop1 and artpop1 align with hivpop and artpop", {
  bw_fp$VERSION='C'; mod <- simmod(bw_fp)
  hp1 <- hivpop_singleage(mod, bw_fp$ss)
  expect_equal(colSums(hp1$hivpop1,,2), colSums(mod$hivpop,,2))
  expect_equal(colSums(hp1$artpop1,,3), colSums(mod$artpop,,3))
  bw_fp$VERSION='K'; mod <- simmod(bw_fp)
  hp1 <- hivpop_singleage(mod, bw_fp$ss)
  expect_equal(colSums(hp1$hivpop1,,2), colSums(mod$hivpop,,2))
  expect_equal(colSums(hp1$artpop1,,3), colSums(mod$artpop,,3))
})
...
test_that("pop and hivpop+artpop are synchronised", {
  ...
  expect_equal(colSums(c$data[,,2,],,2), colSums(c$hivpop,,3) + colSums(c$artpop,,4))
  expect_equal(colSums(r$data[,,2,],,2), colSums(r$hivpop,,3) + colSums(r$artpop,,4))
})

test_that("Model C & new R ouputs are equal", {
  bw_fp$VERSION='C'; c <- simmod(bw_fp)
  bw_fp$VERSION='R'; r <- simmod(bw_fp)
  expect_equal(c$natdeaths, r$natdeaths)
  expect_equal(c$popadjust, r$adj_prob_age)
  expect_equal(c$prev15to49, r$prev15to49)
  expect_equal(c$incid15to49, r$incid15to49)
  expect_equal(c$hivpop, r$hivpop)
  expect_equal(c$artpop, r$artpop)
  expect_equal(c$infections, r$infections)
  expect_equal(c$hivdeaths, r$hivdeaths)
  expect_equal(c$pregprevlag[1:52], r$pregprevlag[1:52])
  expect_equal(c$pregprev[1:52], r$pregprev[1:52])
  # C++ does not save pregprevlag here
  message("hivp_entrants is not save in C++")
  message("r_ts is not save in R")
  expect_equal(c$prev15to49_ts, r$prev15to49_ts[1:length(c$prev15to49_ts)])
  expect_equal(c$incrate15to49_ts, r$incrate15to49_ts[1:length(c$incrate15to49_ts)])
  expect_equal(c$entrantprev, r$entrantprev)
})

test_that("Model C and new C++ ouputs are equal", {
  bw_fp$ss$MODEL <- 1L
  bw_fp$ss$MIX   <- FALSE
  bw_fp$VERSION='C'; c <- simmod(bw_fp)
  bw_fp$VERSION='K'; k <- simmod(bw_fp)
  expect_equal(c$natdeaths, k$natdeaths)
  expect_equal(c$popadjust, k$popadjust)
  expect_equal(c$prev15to49, k$prev15to49)
  expect_equal(c$incid15to49, k$incid15to49)
  expect_equal(c$hivpop, k$hivpop)
  expect_equal(c$artpop, k$artpop)
  expect_equal(c$infections, k$infections)
  expect_equal(c$hivdeaths, k$hivdeaths)
  expect_equal(c$pregprevlag[1:52], k$pregprevlag[1:52])
  expect_equal(c$pregprev[1:52], k$pregprev[1:52])
  expect_equal(c$prev15to49_ts[1:520], k$prev15to49_ts[1:520])
  expect_equal(c$entrantprev, k$entrantprev)
})
...
test_that("Mixing model C++ equal R, no C version", {
  ug$ss$MODEL <- 2L
  ug$ss$MIX   <- TRUE
  ug$VERSION='R'; r <- simmod(ug)
	ug$VERSION='K'; k <- simmod(ug)
  expect_equal(prev(r), prev(k))
  expect_equal(incid(r), incid(k) )
  expect_equal(r$data[,,,], k$data[,,,])
  expect_equal(r$entrantprev, k$entrantprev )
  expect_equal(r$infections, k$infections)
  expect_equal(r$hivpop, k$hivpop)
  expect_equal(r$artpop, k$artpop)
  expect_equal(r$natdeaths, k$natdeaths)
  expect_equal(r$adj_prob_age, k$popadjust)
  expect_equal(r$prev15to49, k$prev15to49)
  expect_equal(r$incid15to49, k$incid15to49)
  expect_equal(r$hivdeaths, k$hivdeaths)
  expect_equal(r$pregprevlag[1:52], k$pregprevlag[1:52])
  expect_equal(r$pregprev[1:52], k$pregprev[1:52])
  expect_equal(r$prev15to49_ts[1:520], k$prev15to49_ts[1:520])
})
