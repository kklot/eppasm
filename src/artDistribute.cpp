...
void popC::art_distribute (const dvec& art_need, const Parameters& p,
  ...
  if (!p.ad.med_cd4init_input[s.year]) {
    ...
    double pr_below = (p.ad.median_cd4init[s.year] - CD4_LO[j]) / (CD4_UP[j] - CD4_LO[j]);
      ...
      for (int sex = 0; sex < s.NG; sex++)
        for (int agr = 0; agr < s.hAG; agr++)
          for (int cd4 = j+1; cd4 < s.hDS; cd4++)
            elig_below[sex] += art_elig_[sex][agr][cd4];

    dvec elig_above(s.NG);
    for (int sex = 0; sex < s.NG; sex++) 
      for (int agr = 0; agr < s.hAG; agr++)
        elig_above[sex] += art_elig_[sex][agr][j] * (1.0 - pr_below);
    if (j > 0)
      for (int sex = 0; sex < s.NG; sex++) 
        for (int agr = 0; agr < s.hAG; agr++)
          for (int cd4 = 0; cd4 <= (j - 1); cd4++)
            elig_above[sex] += art_elig_[sex][agr][cd4];
    ...
    if (j < (s.hDS - 1)) 
      for (int sex = 0; sex < s.NG; sex++)
        for (int agr = 0; agr < s.hAG; agr++)
          for (int cd4 = j + 1; cd4 < s.hDS; cd4++)
            art_init_[sex][agr][cd4] = art_elig_[sex][agr][cd4] * initpr_below[sex];
    
    for (int sex = 0; sex < s.NG; sex++)
      for (int agr = 0; agr < s.hAG; agr++)
        art_init_[sex][agr][j] = art_elig_[sex][agr][j] * initpr_medcat[sex];
    
    if (j == 0)
      for (int sex = 0; sex < s.NG; sex++)
        for (int agr = 0; agr < s.hAG; agr++)
            art_init_[sex][agr][0] = art_elig_[sex][agr][0] * initpr_above[sex];
    else if (j > 0)
      for (int sex = 0; sex < s.NG; sex++)
        for (int agr = 0; agr < s.hAG; agr++)
          for (int cd4 = 0; cd4 <= (j - 1); cd4++)
            art_init_[sex][agr][cd4] = art_elig_[sex][agr][cd4] * initpr_above[sex];
  }
}